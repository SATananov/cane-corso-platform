...
const copyByLocale: Record<'en' | 'bg' | 'it', FaqCopy> = {
  en: {
    ...
    sections: [
      ...
      { id: 'registry-profiles', eyebrow: '02 / Registry', title: 'Registry, profiles, and evidence levels', description: 'Registry is visibility, not an automatic pedigree claim.', items: [
        ...
        { question: 'Can I add a Cane Corso without FCI paperwork?', answer: 'Yes, but the profile must be transparent. USG can present photos, owner history, known parents, family-line notes, and USG observations while clearly showing that official proof is incomplete when documents are missing.' },
      ...
      { id: 'gallery-community-partners', eyebrow: '04 / Gallery + Community + Partners', title: 'Public presence, help requests, and services', description: 'The platform separates beauty, usefulness, and sensitive contact.', items: [
        ...
        { question: 'Why does Community use mediated contact?', answer: 'Sensitive requests such as breeding, puppies, adoption / new home, services, transport, and lost / found help should not become uncontrolled direct contact. USG mediation protects privacy, safety, trust, and quality.', href: '/community', hrefLabel: 'Open Community' },
        { question: 'What are Partners and Services?', answer: 'Partner and service profiles can include trainers, boarding, transport, dog-friendly places, shops, and other Cane Corso-relevant services. Approval and categorization keep the ecosystem useful and trusted.', href: '/partners', hrefLabel: 'Open Partners' },
      ] },
      ...
      { id: 'privacy-safety', eyebrow: '06 / Privacy + safety', title: 'Public visibility and moderation', description: 'Trust needs boundaries, not only design.', items: [
        { question: 'Who decides what becomes public?', answer: 'Public visibility is controlled by the platform path and USG review. Owner profiles, community requests, partner applications, and trust decisions should not become public authority automatically.' },
        { question: 'Why hide or limit some contact details?', answer: 'Direct contact can be risky in breeding, puppies, adoption, transport, services, and lost / found cases. The platform protects users through contact preferences and USG-mediated connection before exposing sensitive details.' },
        { question: 'Can USG reject or ask for changes?', answer: 'Yes. USG review can approve, reject, request changes, publish, archive, or connect parties depending on the area. This protects the platform from confusion, low-quality content, and unsafe public claims.' },
      ] },
    ],
  ...
  bg: {
    ...
    cards: [
      { eyebrow: 'Карта на платформата', title: 'Всяка публична част има отделна роля', description: 'Регистър, Сертификат, Проверка, Галерия, Общност, Партньори и Знания са свързани, но не означават едно и също нещо.', href: '#platform-map', meta: 'USG • Регистър • Знания', icon: 'platform' },
      ...
      { eyebrow: 'Знание за породата', title: 'Официалните източници и историята на възраждането остават видими', description: 'Сериозни стандарти, история и обучителни източници за съдии са свързани директно за отговорно четене.', href: '#official-references', meta: 'FCI • ENCI • AKC • CCAA', icon: 'knowledge' },
    ],
    chips: ['FAQ за цялата платформа', 'Официални граници', 'Проверени източници'],
    priorityTitle: 'Въпросите, които хората задават първо',
    priorityDescription: 'Кратки отговори преди посетителят да се регистрира, да подаде профил, да чете Знания или да използва Общността.',
    priorityItems: [
      ...
      { question: 'Защо има линкове към външни Cane Corso източници?', answer: 'Защото Знания не трябва да бъде затворено само вътре в USG. Официални стандарти, история, обучителни материали за съдии и експертни източници помагат на собствениците да сравняват платформените обяснения със сериозни външни източници.', href: '#official-references', hrefLabel: 'Виж източниците' },
    ],
    ...
    sections: [
      { id: 'platform-map', eyebrow: '01 / Платформа', title: 'Какво е Cane Corso Platform by USG?', description: 'Платформата е екосистема, не една страница със списък.', items: [
        ...
        { question: 'Какво прави всяка основна част?', answer: 'Регистърът публикува прегледани публични профили. Сертификатът записва отделно USG решение за доверие. Проверката потвърждава издаден USG код. Галерията е подбрано визуално представяне. Общността помага с модерирани заявки. Партньорите показват одобрени услуги. Знания образова и обяснява границите.' },
      ] },
      { id: 'registry-profiles', eyebrow: '02 / Регистър', title: 'Регистър, профили и нива на доказуемост', description: 'Регистърът е публична видимост, не автоматична претенция за родословие.', items: [
        { question: 'Какво е USG Регистър?', answer: 'Регистърът е официалната публична профилна част вътре в USG. Публикуван профил означава, че платформата е прегледала и показала профила. Това не означава автоматично официално родословие или USG сертификат.', href: '/registry', hrefLabel: 'Отвори Регистъра' },
        { question: 'Мога ли да добавя Cane Corso без FCI документи?', answer: 'Да, но профилът трябва да бъде прозрачен. USG може да представи снимки, история от собственика, известни родители, бележки за семейна линия и USG наблюдения, като ясно показва, че официалното доказване е непълно, когато документите липсват.' },
      ...
      { id: 'certificate-verify', eyebrow: '03 / Сертификат + Проверка', title: 'Какво наистина означават USG сертификат и Проверка', description: 'Сертификатът трябва да изглежда премиум, но езикът му трябва да остане точен.', items: [
        { question: 'USG сертификатът родословие ли е?', answer: 'Не. USG сертификатът е платформен документ за доверие. Той може да покаже прегледана идентичност, ниво на доказуемост, история на собственика, налични документи, снимки, връзка с Регистъра и код за Проверка. Той не е FCI родословие, клубна регистрация, съдебен доклад, ветеринарен сертификат или официален киноложки документ.', href: '/knowledge/usg-certificate-evidence-levels', hrefLabel: 'Отвори обяснението за сертификата' },
        ...
        { question: 'Какво проверява Проверка?', answer: 'Проверката потвърждава издаден от USG сертификатен код и съответния платформен запис. Тя не проверява FCI бази данни и не заменя официални киноложки регистри.', href: '/verify', hrefLabel: 'Отвори Проверка' },
      ] },
      { id: 'gallery-community-partners', eyebrow: '04 / Галерия + Общност + Партньори', title: 'Публично присъствие, заявки за помощ и услуги', description: 'Платформата разделя красота, полезност и чувствителен контакт.', items: [
        { question: 'Галерията класация ли е?', answer: 'Не. Галерията е подбрано визуално представяне. Тя може да показва силно публично присъствие, но не е класация за стойността на кучето и не заменя Регистъра, Сертификата или официални документи.', href: '/gallery', hrefLabel: 'Отвори Галерията' },
        { question: 'Защо Общността използва USG посредничество?', answer: 'Чувствителни заявки като разплод, малки, осиновяване / нов дом, услуги, транспорт и изгубено / намерено не трябва да стават неконтролиран директен контакт. USG посредничеството пази поверителност, безопасност, доверие и качество.', href: '/community', hrefLabel: 'Отвори Community' },
        { question: 'Какво са Партньори и Услуги?', answer: 'Партньорските и сервизни профили могат да включват треньори, хотели, транспорт, места, подходящи за кучета, магазини и други услуги, свързани с Cane Corso. Одобрението и категоризацията пазят екосистемата полезна и доверена.', href: '/partners', hrefLabel: 'Отвори Партньори' },
      ] },
      { id: 'knowledge-bulgarico', eyebrow: '05 / Знания + Bulgarico', title: 'Знание за породата, официални източници и USG наблюдения', description: 'Знания образова; то не заменя официални стандарти или професионална преценка.', items: [
        { question: 'Какво е Знания?', answer: 'Знания е образователната част за история на Cane Corso, четене на стандарта, отговорно притежание, здравна осведоменост, обучение, пътуване, осиновяване, USG език на доверие и граници на платформата. Тя цитира сериозни източници и разделя официални факти от USG наблюдения.', href: '/knowledge', hrefLabel: 'Отвори Знания' },
      ...
      { id: 'privacy-safety', eyebrow: '06 / Поверителност + безопасност', title: 'Публична видимост и модерация', description: 'Доверието има нужда от граници, не само от дизайн.', items: [
        { question: 'Кой решава какво става публично?', answer: 'Публичната видимост се контролира от платформения процес и USG преглед. Заявките от собственици, общностните записи, партньорските заявки и решенията за доверие не трябва автоматично да се превръщат в публична власт.' },
        { question: 'Защо някои контакти се скриват или ограничават?', answer: 'Директният контакт може да бъде рисков при разплод, малки, осиновяване, транспорт, услуги и изгубено / намерено. Платформата пази потребителите чрез предпочитания за контакт и USG посредничество преди да показва чувствителни данни.' },
        { question: 'Може ли USG да откаже или да поиска промени?', answer: 'Да. USG прегледът може да одобри, откаже, поиска промени, публикува, архивира или свърже страни според конкретната част на платформата. Това пази платформата от объркване, нискокачествени записи и небезопасни публични твърдения.' },
      ] },
    ],
    ...
    expertsTitle: 'Имена за проучване при историята на възраждането', expertsDescription: 'USG представя историческите имена внимателно: като хора, кучета и организации, които често се свързват с възстановяване, документиране, съдийско обучение или породни образователни източници — не като платформено одобрение или частни твърдения.',
    experts: ['Dr. Paolo Breber — широко свързван с възстановяването и документирането на оцелели кучета от тип Cane Corso.', 'Società Amatori Cane Corso (SACC) — породното общество, свързано с формалното възстановяване и ранния период на стандартизиране.', 'Giovanni Bonnetti, Vito Indiveri, Stefano Gandolfi и братята Malavasi — имена, които се срещат в обобщения за съвременната история на възраждането.', 'Antonio Morsiani и Stefano Gandolfi — имена, свързани със специализирана породна литература и обучителни материали за съдии.', 'Basir и Babak — кучета, често споменавани като важни ранни примери за модерния стандарт.'],
  ...
  it: {
    ...
    cards: [
      { eyebrow: 'Mappa piattaforma', title: 'Ogni superficie pubblica ha un ruolo separato', description: 'Registro, Certificato, Verifica, Galleria, Comunità, Partner e Conoscenze sono collegati, ma non significano la stessa cosa.', href: '#platform-map', meta: 'USG • Registro • Conoscenza', icon: 'platform' },
      ...
      { eyebrow: 'Educazione razza', title: 'Fonti ufficiali e storia della rinascita restano visibili', description: 'Standard, storia e riferimenti seri per la formazione dei giudici sono collegati direttamente per una lettura responsabile.', href: '#official-references', meta: 'FCI • ENCI • AKC • CCAA', icon: 'knowledge' },
    ],
    ...
    priorityItems: [
      ...
      { question: 'Perché ci sono link a fonti esterne?', answer: 'Perché Knowledge non dovrebbe restare chiuso dentro USG. Standard ufficiali, storia, formazione dei giudici e materiali specialistici aiutano a confrontare le spiegazioni della piattaforma con fonti serie.', href: '#official-references', hrefLabel: 'Vedi fonti' },
    ],
    ...
    sections: [
      { id: 'platform-map', eyebrow: '01 / Piattaforma', title: 'Che cos’è Cane Corso Platform by USG?', description: 'La piattaforma è un ecosistema, non una semplice lista.', items: [
        { question: 'Che cosa significa Unico Suo Genere?', answer: 'Unico Suo Genere significa unico nel suo genere. Nella piattaforma è un modo premium per organizzare identità Cane Corso, storia del proprietario, revisione, fiducia, educazione e supporto della comunità, senza presentare USG come autorità cinofila ufficiale.', href: '/platform', hrefLabel: 'Apri Platform' },
        { question: 'La piattaforma è solo per la Bulgaria?', answer: 'No. USG nasce da un creatore bulgaro e da osservazioni bulgare, ma l’idea è internazionale e può aiutare proprietari, allevatori, partner e comunità Cane Corso responsabili anche in altri paesi.' },
      ...
      { id: 'registry-profiles', eyebrow: '02 / Registry', title: 'Registry, profili e livelli di evidenza', description: 'Registry è visibilità pubblica, non una pretesa automatica di pedigree.', items: [
        { question: 'Che cos’è USG Registry?', answer: 'Registry è il livello ufficiale dei profili pubblici dentro USG. Un profilo pubblicato significa che la piattaforma lo ha revisionato e reso visibile. Non significa automaticamente pedigree ufficiale o USG Certificate.', href: '/registry', hrefLabel: 'Apri Registry' },
        { question: 'Posso aggiungere un Cane Corso senza documenti FCI?', answer: 'Sì, ma il profilo deve essere trasparente. USG può presentare foto, storia del proprietario, genitori conosciuti, note di linea familiare e osservazioni USG, mostrando chiaramente quando la prova ufficiale è incompleta.' },
      ...
      { id: 'gallery-community-partners', eyebrow: '04 / Galleria + Comunità + Partner', title: 'Presenza pubblica, richieste di aiuto e servizi', description: 'La piattaforma separa bellezza, utilità e contatto sensibile.', items: [
        { question: 'Gallery è una classifica?', answer: 'No. Gallery è una presentazione visuale curata. Può evidenziare una forte presentazione pubblica, ma non è una classifica del valore del cane e non sostituisce Registry, Certificate o documenti ufficiali.', href: '/gallery', hrefLabel: 'Apri Gallery' },
        { question: 'Perché Community usa contatto mediato?', answer: 'Richieste sensibili come riproduzione, cuccioli, adozione / nuova casa, servizi, trasporto e smarrito / trovato non dovrebbero diventare contatto diretto non controllato. La mediazione USG protegge privacy, sicurezza, fiducia e qualità.', href: '/community', hrefLabel: 'Apri Community' },
        { question: 'Che cosa sono Partner e Servizi?', answer: 'Partner e servizi possono includere addestratori, pensioni, trasporto, luoghi pet-friendly, negozi e altri servizi rilevanti per Cane Corso. Approvazione e categorizzazione mantengono l’ecosistema utile e affidabile.', href: '/partners', hrefLabel: 'Apri Partners' },
      ] },
      { id: 'knowledge-bulgarico', eyebrow: '05 / Conoscenza + Bulgarico', title: 'Educazione razza, fonti ufficiali e osservazioni USG', description: 'Knowledge educa; non sostituisce standard ufficiali o valutazioni professionali.', items: [
        { question: 'Che cos’è Knowledge?', answer: 'Knowledge è il livello educativo per storia Cane Corso, lettura dello standard, responsabilità del proprietario, salute, addestramento, viaggio, adozione, linguaggio di fiducia USG e confini della piattaforma. Cita fonti serie e separa fatti ufficiali da osservazioni USG.', href: '/knowledge', hrefLabel: 'Apri Knowledge' },
      ...
      { id: 'privacy-safety', eyebrow: '06 / Privacy + sicurezza', title: 'Visibilità pubblica e moderazione', description: 'La fiducia ha bisogno di confini, non solo design.', items: [
        { question: 'Chi decide cosa diventa pubblico?', answer: 'La visibilità pubblica è controllata dal percorso della piattaforma e dalla revisione USG. Profili dei proprietari, richieste della comunità, candidature partner e stati di fiducia non diventano automaticamente autorità pubblica.' },
        { question: 'Perché alcuni contatti sono nascosti o limitati?', answer: 'Il contatto diretto può essere rischioso in riproduzione, cuccioli, adozione, trasporto, servizi e smarrito / trovato. La piattaforma protegge gli utenti tramite preferenze di contatto e mediazione USG prima di mostrare dati sensibili.' },
        { question: 'USG può rifiutare o chiedere modifiche?', answer: 'Sì. La revisione USG può approvare, rifiutare, chiedere modifiche, pubblicare, archiviare o collegare le parti secondo l’area specifica. Questo protegge da confusione, contenuti di bassa qualità e affermazioni pubbliche non sicure.' },
      ] },
    ],
    ...
    expertsTitle: 'Nomi da ricercare nella storia della rinascita', expertsDescription: 'USG presenta i nomi storici con attenzione: come persone, cani e organizzazioni spesso collegate a recupero, documentazione, valutazione cinofila o fonti educative sulla razza — non come approvazione della piattaforma.',
    experts: ['Dr. Paolo Breber — ampiamente collegato al recupero e alla documentazione di cani di tipo Cane Corso sopravvissuti.', 'Società Amatori Cane Corso (SACC) — società di razza collegata al recupero formale e al primo periodo di standardizzazione.', 'Giovanni Bonnetti, Vito Indiveri, Stefano Gandolfi e i fratelli Malavasi — nomi presenti in riassunti della storia moderna del recupero.', 'Antonio Morsiani e Stefano Gandolfi — nomi collegati a letteratura specialistica di razza e materiali per la formazione dei giudici.', 'Basir e Babak — cani spesso menzionati come esempi iniziali importanti per lo standard moderno.'],
  ...
  return (
    <PageShell eyebrow={copy.eyebrow} title={copy.title} description={copy.description} cards={copy.cards} actionLabel={actionLabel} accentLabel={copy.eyebrow} helpHref="/guide?topic=overview#overview" helpLabel={helpLabel} visualSrc="/brand/icons/brand-icon.png" visualAlt="USG FAQ clarity symbol" visualFit="contain" heroChips={copy.chips} variant="knowledge">
      ...
      <nav className="content-card platform-faq-nav" aria-label={copy.navTitle}>
        <span className="eyebrow-label">{locale === 'bg' ? 'Карта на FAQ' : locale === 'it' ? 'Mappa FAQ' : 'FAQ map'}</span><h2>{copy.navTitle}</h2>
        <div className="platform-faq-nav-grid">{copy.sections.map((section) => <a key={section.id} href={`#${section.id}`}><span>{section.eyebrow}</span>{section.title}</a>)}<a href="#official-references"><span>{locale === 'bg' ? 'Източници' : locale === 'it' ? 'Fonti' : 'Sources'}</span>{copy.referencesTitle}</a></div>
      </nav>
      ...
      <section className="content-card platform-faq-references" id="official-references" aria-labelledby="official-references-title">
        <div className="platform-faq-heading platform-faq-heading--split"><div><span className="eyebrow-label">{locale === 'bg' ? 'Официални и специализирани източници' : locale === 'it' ? 'Fonti ufficiali e specialistiche' : 'Official and specialist sources'}</span><h2 id="official-references-title">{copy.referencesTitle}</h2><p>{copy.referencesDescription}</p></div><p className="platform-faq-source-note">{copy.referencesNote}</p></div>
      ...
      <section className="content-card platform-faq-experts" aria-labelledby="faq-experts-title"><div className="platform-faq-heading"><span className="eyebrow-label">{locale === 'bg' ? 'Път за четене на историята' : locale === 'it' ? 'Percorso di lettura storica' : 'Breed history reading path'}</span><h2 id="faq-experts-title">{copy.expertsTitle}</h2><p>{copy.expertsDescription}</p></div><ul className="platform-faq-expert-list">{copy.experts.map((expert) => <li key={expert}>{expert}</li>)}</ul></section>
      <section className="content-card platform-faq-boundary" aria-labelledby="faq-boundary-title"><div className="platform-faq-heading"><span className="eyebrow-label">{locale === 'bg' ? 'Граница на USG доверие' : locale === 'it' ? 'Confine di fiducia USG' : 'USG trust boundary'}</span><h2 id="faq-boundary-title">{copy.boundaryTitle}</h2></div><ul className="platform-faq-boundary-list">{copy.boundaryItems.map((item) => <li key={item}>{item}</li>)}</ul></section>
    </PageShell>
  );
}
