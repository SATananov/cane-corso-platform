...
const copyByLocale = {
  ...
  bg: {
    ...
    qualification: {
      candidate_for_usg_review: 'Може да се квалифицира за USG преглед, когато човек/админ потвърди доказателствата.',
      collect_more_data: 'Събери още данни, преди резултатът да се използва за сериозна подготовка за преглед.',
      development_only: 'Само сигнал за развитие. Кученце не трябва да получава финална adult FCI оценка.',
    ...
    sections: {
      height_weight: ['Височина и тегло', 'Сравнява височина при холката и тегло с adult FCI ориентира според пола.'],
      ...
      faults_boundary: ['Граница за faults', 'Сериозни и дисквалифициращи faults не могат да се откриват безопасно само от полета във форма.'],
    ...
    evidenceKeys: {
      ...
      adult_height_range: 'adult височина според пола',
      adult_weight_range: 'adult тегло според пола',
      ...
      puppy_not_final_standard: 'развитието на кученце не е финална adult FCI оценка',
      disqualifying_faults_not_auto_detected: 'faults не се откриват автоматично безопасно',
      human_review_required: 'нужен е човешки преглед',
    },
    ...
    <section className="content-card fci-conformity-panel" aria-label={copy.title}>
      ...
      <div className="fci-conformity-panel__summary">
        <div className="fci-conformity-panel__score">
          ...
          <strong>{document.scores.overall}%</strong>
          <small>{copy.confidence}: {document.confidence}</small>
        </div>
          ...
          return (
            <article className={sectionStatusClass(section.status)} key={section.key}>
              ...
              <div className="fci-conformity-section__meta">
                ...
                <span>{copy.confidence}: {section.confidence}</span>
